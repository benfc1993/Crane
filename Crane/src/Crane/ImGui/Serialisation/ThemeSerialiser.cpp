...
namespace Crane {
	ThemeSerialiser::ThemeSerialiser(ImGuiTheme& theme)
		: m_Theme(theme)
	{

	}

	void ThemeSerialiser::Serialise(const std::string filePath, const std::string name)
	{
		YAML::Emitter out;
		out << YAML::BeginMap;
	...
	void DeSerialiseColor(glm::vec4 color, float(&themeValue)[4])
	{
		for (int i = 0; i < 4; i++)
			themeValue[i] = color[i];
	}

	bool ThemeSerialiser::Deserialise(const std::string filePath)
	{
		...
		CR_CORE_TRACE("Deserialising theme {0}", m_Theme.Name);
