...
namespace Crane {
	...
	Ref<Project> Project::Load(const std::filesystem::path& filepath)
	{
		...
		ProjectSerialiser serialiser = ProjectSerialiser(project);
		if (serialiser.Deserialise(filepath))
		{
	...
	{
		ProjectSerialiser serialiser = ProjectSerialiser(s_ActiveProject);
		return serialiser.Serialise(filepath);
	}
